...
// Side-effect import: this is what puts the stylesheet into the webpack module graph.
// `pbiviz.json`'s `style` field is only read by the official `pbiviz package` command,
// which this repository does not use, so without this import no CSS is ever compiled
// and every rule in style/visual.less is dead.
...
export class Visual implements IVisual {
   ...
   *
   * Every band declares `top: 0` in CSS, which is correct for a single band. As soon
   * as the Period hierarchy is expanded there are two or more bands, and they would
   * all stick at 0 and collapse onto the first one the moment the matrix is scrolled.
   * Offsets can only come from measurement, so this reads layout once per render and
   * only when there is more than one band.
   */
