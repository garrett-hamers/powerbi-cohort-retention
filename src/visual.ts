...
export class Visual implements IVisual {
  ...
  private renderColumnHeaders(model: CohortModel): void {
    ...
    for (let level = 0; level <= maxLevel; level += 1) {
      ...
      if (level === 0) {
        ...
        corner.setAttribute("aria-rowspan", String(maxLevel + 1));
      ...
      while (index < model.columns.length) {
        ...
        header.setAttribute("aria-colspan", String(end - index));
        header.setAttribute("aria-rowspan", "1");
  ...
  private renderRow(
    ...
    row.cells.forEach((cell, columnIndex) => {
      ...
      if (cell.status === "future" || cell.status === "missing") {
