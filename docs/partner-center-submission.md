...
| Visual delivery | Exact `.pbiviz` archive entries extracted under `Report/CustomVisuals/<GUID>/` and registered as a `CustomVisual` resource package, not `publicCustomVisuals` |
...
| SHA-256 | `e582bc85aaa006343fde4b251f79ca62c2cc8bbdc2fa86d0a12d34a58c0e4aa7` |
| Size | 22,187 bytes |
...
| --- | --- | --- |
| Windows, Node 24.17.0, zlib 1.3.1-e00f703 | `e582bc85aaa006343fde4b251f79ca62c2cc8bbdc2fa86d0a12d34a58c0e4aa7` | 22,187 bytes |

**Confirmed identical across three consecutive local builds**, so
`e582bc85aaa006343fde4b251f79ca62c2cc8bbdc2fa86d0a12d34a58c0e4aa7`
at 22,187 bytes is the value to publish in the release manifest, under
...
first correct two-entry layout, superseded by the GUID change). Every hash before those
...
**That list is machine-read.** `scripts/doc-hash-gate.js`, called from
`scripts/certification-audit.js` inside `npm run package`, parses the paragraph above as the
register of hashes that are historical on purpose, then compares every other SHA-256 and
byte size in this file and in `CHANGELOG.md` against artifacts hashed from disk in the same
run. A value that matches no current artifact and is not registered above fails the build
with its file and line. So when the bytes change, the old hash must be *moved* into that
paragraph rather than simply overwritten — otherwise it is indistinguishable from a value
that went stale, which is exactly the case this gate exists to catch. The gate also requires
the current packaged hash to be recorded in both files, so deleting a stale value instead of
...
| `samples/…/CustomVisuals/<GUID>/resources/<GUID>.pbiviz.json` | `content.css` inside the matching PBIVIZ resource | 5,167 |
...
| `npm test -- tests/doc-hashes.test.ts` | Pins the recorded-value drift gate: that a stale hash, a byte size left behind on another table row, a hash attached to the wrong artifact, and a missing record are each reported with file and line, and that registered historical hashes are not. |
